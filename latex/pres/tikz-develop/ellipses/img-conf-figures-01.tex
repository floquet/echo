...
\begin{tikzpicture}
    \begin{axis}[
        ...
        % Gray line from (-1,-1) to (1,1)
        \addplot[color=gray, line width=0.5pt] coordinates {(-1,-1) (1,1)};

        % Ellipse defined by c0, c1, sigma0, sigma1
        % Values are placeholders; in a live TeX document, these would be \pgfmathparse values from sliders
        \def\czero{0.3}
        \def\cone{0.5}
        \def\sigmazero{0.2}
        \def\sigmaone{0.1}

        \draw[blue, thick] (\czero, \cone) ellipse [x radius=\sigmazero, y radius=\sigmaone];
        
        % Center point
        \filldraw[black] (\czero, \cone) circle (1pt);
   \end{tikzpicture}
