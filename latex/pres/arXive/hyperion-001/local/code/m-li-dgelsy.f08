...
! https://www.netlib.org/lapack/explore-3.1.1-html/dgelsy.f.html
module mLapackInterfaceDGELSY
    ...
    end interface lapack_dgelsy

contains

    subroutine echo_dgelsy ( m, n, nrhs, A, lda, b, ldb, jpvt, rcond, rank, work, lwork, info )
        use mPrecisionDefinitions, only : rp
        integer,                                              intent ( in )    :: m, n, nrhs, lda, ldb, lwork
        integer,                                              intent ( out )   :: rank, info
        integer,            dimension ( 1 : n ),              intent ( inout ) :: jpvt
        ...
        integer :: col = 0, row = 0, io_work = 0
            ...
            write ( * , * ) "Slot arguments to dgelsy:"
    ...
    end subroutine echo_dgelsy

    subroutine error_dgelsy ( info_value, message )
        use mPrecisionDefinitions, only : kindA
        integer, intent ( in ) :: info_value
        character ( kind = kindA, len = * ), intent ( in ), optional :: message
        character ( kind = kindA, len = 6 ), parameter :: iam = "DGELSY"
    ...
    end subroutine error_dgelsy

end module mLapackInterfaceDGELSY
